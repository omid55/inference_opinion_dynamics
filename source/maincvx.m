...
%% Learning by Yamagishi
yama_tt = Yamagishi(data, net, initOp);
...
[~,tmp] = sort(soc_pow,'descend');
tmps = tmp(1:InformedAgentsSize)';
tmpl = floor(length(tmps)/2);
tmpl2 = floor(length(tmps)/4);
...
% end
[~,SP_tar] = sort(SP,'descend');
targets = SP_tar(1:InformedAgentsSize)';


% Yamagishi's unguaranteed method
[~,sppp] = sort(yama_tt,'descend');
anotherTar = sppp(1:InformedAgentsSize)';
...
% % % highBetweenness = GiveGoods(betw,InformedAgentsSize);
anotherTar(1:tmpl2) = tmps(1:tmpl2);
...
for i = 1 : N
    ...
    f2 = (sum(indegs(net(i,:) == 1)) / max(indegs));
...
ranks = HITS(net);
[~,spp] = sort(ranks,'descend');
rankTar = spp(1:InformedAgentsSize)';


%% Influence maximization simulations
randD = zeros(times,MaxSimulationIteration+1);
rankD = zeros(times,MaxSimulationIteration+1);
anotherD = zeros(times,MaxSimulationIteration+1);
...
for time = 1 : times
    ...
    resRank = VoterSimulationMethod(op,net,rankTar,MaxSimulationIteration,K,desiredOp,soc_pow,InformedAgentsStrength);
    resAnother = VoterSimulationMethod(op,net,anotherTar,MaxSimulationIteration,K,desiredOp,soc_pow,InformedAgentsStrength);
    ...
    rankD(time,:) = resRank.desOpPop;
    anotherD(time,:) = resAnother.desOpPop;
...
errorbar(times,myScale*mean(rankD),myScale*std(rankD)/sqrt(size(rankD,1)),'k');
errorbar(times,myScale*mean(anotherD),myScale*std(anotherD)/sqrt(size(anotherD,1)),'m');
...
leg = legend('EPN','PageRank','Yamagishi','High Degree','High Betweenness','High Closeness','Proposed','Location','Southeast');
set(leg,'FontSize',7);
